...
log 用来续做工作，不用来开启新工作。新工作要先根据请求和仓库结构写出范围，再读 log 和 decision：agent 如果先读到一长串围绕某个部分的工作记录，往往会把一个关于整个系统的请求收窄到那一部分（decision 0019）。
