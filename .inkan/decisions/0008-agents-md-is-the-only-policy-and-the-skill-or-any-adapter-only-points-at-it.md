...
### 2026-09-17T02:39:23.353Z, outcome 2026-09-17-0237-gbb2

Status: accepted -> accepted

Protocol 11 (2026-09-17) implements 0018: the generated block carries <!-- inkan-mode: repo --> or <!-- inkan-mode: local -->. Repo mode keeps the protocol 10 publication duty: commit the outcome record with the work, include the Inkan-Outcome trailer, and commit .inkan/ with the code. Local-only mode keeps the seal, amend, close, re-anchor, and closed-is-final rules, and changes only that duty: keep .inkan/ on this checkout, do not commit it, and do not include an Inkan-Outcome trailer. init --local writes local mode; init --repo writes repo mode; omitting both keeps the current mode, or repo when there is no block yet. --local together with --repo is refused. Hiro asked for this on 2026-09-17 because some checkouts cannot or should not publish the record, while the default remains that records travel with the code. The mode is the stamp and the generated text in AGENTS.md, not a sidecar or environment variable, per 0008. init upgrades generated protocols 1 through 10 to protocol 11 repo mode, upgrades a generated local-only block in place, and still refuses hand-edited blocks.

### 2026-09-24T03:54:56.755Z, outcome 2026-09-24-0353-z0vq
